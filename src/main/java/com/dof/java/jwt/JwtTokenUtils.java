...
public interface JwtTokenUtils {
  ...
  /**
   * Generate a self signed JWT to request and Identity Token (OpenID conform) or 
   * an Access Token (Oauth2 conform) to GCP end point.
   * Required properties parameter configurable by builder methods are:
   *  {@link JwtTokenUtilsBuilder#setTargetTokenType(TargetTokenType)}
   * In case of Identity Token:
   *  {@link JwtTokenUtilsBuilder#setTargetServiceUrl(String)}
   *
